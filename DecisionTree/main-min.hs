...
bagTrain bags exs fs = map (flip train fs . map (axs V.!)) . take bags $ chunksOf bag rs
	where
		bag = n `div` 2
		n = length exs - 1
		axs = V.fromList exs 
...
main = 	do
	...
	pruning <- load "-prune" 
	testing <- load "-test" 
	learning <- fromMaybe (return []) . fmap (fmap (map V.fromList.readCSV).readFile) $ lookup "-learn" opts
